...
import { getUserAlbum } from '../../../../layers/business/database/album';
...
const handler: ValidatedEventAPIGatewayProxyHandlerV2<typeof schema> = async (
    ...
    logger.info(`${MiddlewarePhases.during} ${context.functionName}`, {
        action: 'Retrieving an user album from DB',
    ...
    // Get an user album item from DB
    const album = await getUserAlbum(context.userId, event.body);

    logger.info(`${MiddlewarePhases.during} ${context.functionName}`, {
        action: 'User album retrieved from DB',
        items: album,
        functionName: context.functionName,
        requestId: context.awsRequestId,
        timestamp: new Date().toISOString(),
